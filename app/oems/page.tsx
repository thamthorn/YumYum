...
export default function OEMList() {
  ...
  const oems = selectedIndustry ? getOEMsByIndustry(selectedIndustry) : [];

  const handleViewProfile = (oemId: number) => {
    if (!isAuthenticated()) {
      toast({
        description: COPY.toasts.needLoginProfile,
      });
      router.push(
        `${ROUTES.login}?next=${encodeURIComponent(ROUTES.oemProfile(oemId))}`
      );
    } else {
      router.push(ROUTES.oemProfile(oemId));
    }
  };
                  ...
                  {oems.map((oem) => (
                    ...
                    >
                      <div className="space-y-4">
                        ...
                        {/* CTA */}
                        <Button
                          onClick={() => handleViewProfile(oem.id)}
                          className="w-full"
                        >
