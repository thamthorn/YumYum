...
import { getVerifiedBadgeVariant } from "@/data/helpers";
import type { OEM } from "@/data/types";
import {
  Factory,
  Home,
  MapPin,
...
export default function ResultsList({ initialOems }: ResultsListProps) {
  ...
  const allCerts = useMemo(() => {
    ...
    initialOems.forEach((o) =>
      o.certifications.forEach((c) => set.add(c.name))
    );
    return Array.from(set);
  }, [initialOems]);

  const filtered = useMemo(() => {
    let list = initialOems.filter((o) => {
    ...
    return list;
  }, [initialOems, search, moqRange, selectedScale, selectedCerts, sortBy]);
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      <div className="flex flex-wrap items-start gap-2 mb-2">
                        <h3 className="text-xl font-semibold">{oem.name}</h3>
                        <Badge
                          variant={getVerifiedBadgeVariant(oem.verification)}
                        >
                  ...
                  <div className="flex lg:flex-col gap-2 lg:w-40">
                    <Button className="flex-1" asChild>
                      <Link href={ROUTES.oemProfile(oem.id)}>View Profile</Link>
                    </Button>
